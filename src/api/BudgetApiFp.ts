...
import { BudgetApiFetchParamCreator } from './BudgetApiFetchParamCreator';
...
export const BudgetApiFp = function(configuration?: Configuration) {
  return {
    ...
    budgetGetBudgetSummaryData(id: string, options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<BudgetSummaryGetWebResponseModel> {
      const localVarFetchArgs = BudgetApiFetchParamCreator(configuration).budgetGetBudgetSummaryData(id, options);
